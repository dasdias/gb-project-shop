...
const app = new Vue({
  ...
  data: {
    ...
    imgElem: '/images/card/card-3.jpg',
    iconCard: '/images/cart.svg',
    products: [],
    imgProduct: 'https://placehold.it/200x150',
    totalPriceQuantity: 0,
  },
  components: { errorconnect },
  methods: {
    getJson(url) {
      return fetch(url)
    ...
    addProduct(item) {
      ...
      this.getJson(`${API}/addToBasket.json`).then((data) => {
        if (find) {
          this.putJson(`/api/cart/${find.id_product}`, { quantity: 1 }).then((data) => {
            if (data.result === 1) {
              find.quantity++;
            }
            this.totalQuantity();
    ...
    remove(item) {
      if (item.quantity > 1) {
        this.putJson(`/api/cart/${item.id_product}`, { quantity: -1 }).then((data) => {
          if (data.result) {
            item.quantity--;
          }
          this.totalQuantity();
        ...
        this.delJson(`/api/cart/${item.id_product}`, item).then((data) => {
          if (data.result) {
    ...
    totalQuantity() {
      if (this.cartItems.length) {
        this.cartItems.reduce((acc, item) => {
          return (this.totalPriceQuantity = item.quantity + acc);
        }, 0);
      } else {
        this.totalPriceQuantity = 0;
      }
    },
  },
  mounted() {
